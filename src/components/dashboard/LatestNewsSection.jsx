// src/components/dashboard/LatestNewsSection.js
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {latestPosts.length > 0 ? (
          <>
            ...
            <div className={`grid grid-cols-1 ${latestPosts.length >= 2 ? 'sm:grid-cols-2' : ''} ${latestPosts.length >= 3 ? 'lg:grid-cols-3' : ''} gap-6 sm:gap-8 mb-8 sm:mb-10`}>
              {latestPosts.map((post) => (
                <NewsCard key={post.id} post={post} />
              ))}
            </div>
            
            {/* Tombol "Semua Berita" di bagian bawah */}
            <div className="text-center">
              <Link
                href="/info"
                className="inline-flex items-center justify-center text-sm sm:text-base text-indigo-600 hover:text-indigo-800 font-semibold py-2.5 px-5 sm:py-3 sm:px-6 rounded-lg transition-colors duration-300 group bg-indigo-50 hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
              >
                Semua Berita
          ...
          <div className="bg-indigo-50 rounded-xl p-6 sm:p-8 text-center shadow-sm border border-indigo-100">
            ...
            <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2 mb-4 sm:mb-6">
              Nantikan update dari kami atau lihat semua berita.
            </p>
            
            {/* Tombol "Semua Berita" di state kosong */}
            <Link
              href="/info"
              className="inline-flex items-center justify-center text-sm sm:text-base text-indigo-600 hover:text-indigo-800 font-semibold py-2.5 px-5 sm:py-3 sm:px-6 rounded-lg transition-colors duration-300 group bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 border border-indigo-200"
            >
              Semua Berita
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5 ml-2 transition-transform duration-300 group-hover:translate-x-1" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" />
