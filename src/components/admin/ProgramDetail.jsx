  ...
  return (
    <div className="w-full bg-white shadow-lg rounded-xl overflow-hidden">
      ...
      <div className="relative h-48 sm:h-64 md:h-72 w-full">
        ...
        <div className="absolute bottom-0 left-0 p-4 sm:p-6 md:p-8 text-white">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0">
            <span className={`px-3 py-1 text-xs sm:text-sm text-center font-medium rounded-full ${getStatusBadgeClass(program.status)}`}>
              {program.status}
            </span>
      ...
      <div className="p-4 sm:p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:gap-8">
            ...
            <div className="mb-8 md:mb-10">
              ...
              <div className="flex flex-col space-y-2 sm:space-y-3">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-600 text-sm sm:text-base">Tanggal Mulai: <strong>{formatDate(program.startDate)}</strong></span>
                </div>
                <div className="flex items-center">
                  <svg className="h-5 w-5 sm:h-6 sm:w-6 text-gray-500 mr-2 sm:mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  ...
                  <span className="text-gray-600 text-sm sm:text-base">Tanggal Selesai: <strong>{formatDate(program.endDate)}</strong></span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-64 shrink-0 mt-6 md:mt-0">
            <div className="bg-gray-50 p-4 sm:p-5 rounded-lg mb-4 sm:mb-6 shadow-sm">
              ...
              {program.division && (
                <p className="text-xs sm:text-sm text-gray-600 line-clamp-3">{program.division.description}</p>
              )}
            </div>
        ...
        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-4 mt-8 md:mt-10 border-t pt-4 sm:pt-6">
          <Link href="/admin/programs" className="px-4 sm:px-5 py-2 sm:py-2.5 border border-gray-300 text-gray-700 rounded-lg text-center hover:bg-gray-50 transition-all font-medium text-sm sm:text-base">
            Kembali
          </Link>
          <Link href={`/admin/programs/edit/${program.id}`} className="px-4 sm:px-5 py-2 sm:py-2.5 bg-indigo-600 text-white text-center rounded-lg hover:bg-indigo-700 transition-all font-medium text-sm sm:text-base">
            Edit Program
          </Link>
          <button
            onClick={() => setIsDeleteModalOpen(true)}
            className="px-4 sm:px-5 cursor-pointer py-2 sm:py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all font-medium text-sm sm:text-base"
          >
            Hapus
          </button>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-4 sm:p-6 transform transition-all">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-3">
              ...
              <button 
                ...
                className="px-4 sm:px-5 cursor-pointer py-2 sm:py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all font-medium text-sm sm:text-base disabled:bg-red-400" 
                disabled={isDeleting}
              >
