...
const EventsWithFilter = ({ initialEvents }) => {
  ...
  useEffect(() => {
    if (initialEvents) {
      ...
      setEvents([]); // Set ke array kosong jika tidak ada initialEvents
  ...
  const filteredEvents =
    filter === "ALL"
      ? events
      : events.filter((event) => event.status === filter); // Perbandingan akan menggunakan nilai filter yang benar
  ...
  const filterOptions = [
    ...
    { id: "COMPLETED", label: "Telah Selesai" } // Ubah "PAST" menjadi "COMPLETED"
  ];
  ...
  return (
    <div className="min-h-screen py-12 sm:py-16 mt-8 px-4 sm:px-6 lg:px-8 bg-slate-50">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {filterOptions.map((statusOption) => ( // Ganti nama variabel agar tidak bentrok dengan 'status' event
            <motion.button
              key={statusOption.id}
              onClick={() => setFilter(statusOption.id)}
        ...
        {isLoading ? (
          ...
          >
            ...
            <p className="text-slate-600 text-sm">
              ...
              {filter === "ALL" 
                ? "tersedia" 
                // Logika untuk teks state kosong disesuaikan agar lebih dinamis
                : (filterOptions.find(f => f.id === filter)?.label.toLowerCase() || "dipilih").replace(" acara", "")
              }.
            </p>
          </motion.div>
        ) : (
          <AnimatePresence mode="wait">
            ...
            >
              {filteredEvents.map((event, index) => (
                // Pastikan EventCard menerima prop 'status' dengan benar dan menampilkannya
